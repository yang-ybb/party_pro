		...
		if(currentUserId == null || currentUser == null ||
		   currentUser.getId() != currentUserId || currentUser.getPermission() < 1 ) {
			...
			if (currentUser.getPermission() == 1 && currentUser.getPartybranchid() != user.getPartybranchid()) {
				...
				if (User.checkUserChange(updateUserId, type)) {
					return "success";
				}
				else {
					message = "审核失败，请重试！";
		...
		if(currentUserId == null || currentUser == null ||
		   currentUser.getId() != currentUserId || currentUser.getPermission() < 1 ) {
			...
			if(User.findByStudentId(studentid) != null) {
				message = "该学号已经存在，请直接登录。";
				return "false";
			}
			User addUser = generateUpdateInfoUser(new User());
			addUser.setPartybranchid(partybranchid);
			Commit addCommit = generateUpdateCommit(new Commit());
			if(User.createUser(addUser, addCommit)) {
